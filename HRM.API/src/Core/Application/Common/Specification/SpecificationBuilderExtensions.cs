...
public static class SpecificationBuilderExtensions
{
    public static ISpecificationBuilder<T> SearchBy<T>(this ISpecificationBuilder<T> query, BaseFilter filter) =>
        query
            .SearchByKeyword(filter.Keyword, filter.SearchWithColumns)
    ...
    public static IOrderedSpecificationBuilder<T> SearchByKeyword<T>(
        ...
        string? keyword, string[]? searchWithColumns) =>
        specificationBuilder.AdvancedSearch(new Search { Keyword = keyword, SearchWithColumns = searchWithColumns });
        ...
        Search? search)
    {
        if (!string.IsNullOrEmpty(search?.Keyword))
        {
            ...
            {
                if (search.SearchWithColumns != null && search.SearchWithColumns.Count() > 0)
                {
                    // search with specific fields (only first level)
                    ...
                    {
                        if (search.SearchWithColumns.Where(x => x.ToLower().Trim() == property.Name.ToLower()).Any())
                        {
                            var paramExpr = Expression.Parameter(typeof(T));
