...
internal partial class UserService
{
    ...
    public async Task<string> ConfirmEmailAsync(string userId, string code, string tenant, CancellationToken cancellationToken)
    {
        ...
        _ = user ?? throw new InternalServerException(string.Format(_localizer["profile.emailconfirmerror"], "Email"));

        code = Encoding.UTF8.GetString(WebEncoders.Base64UrlDecode(code));
        var result = await _userManager.ConfirmEmailAsync(user, code);

        return result.Succeeded
            ? string.Format(_localizer["profile.emailconfirm"], user.Email)
            : throw new InternalServerException(string.Format(_localizer["profile.emailconfirmerror"], user.Email));
    }
