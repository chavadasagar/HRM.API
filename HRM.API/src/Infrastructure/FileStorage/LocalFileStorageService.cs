...
public class LocalFileStorageService : IFileStorageService
{
    ...
    public async Task<string> UploadAsync<T>(FileUploadRequest? request, string folderName, FileType supportedFileType, CancellationToken cancellationToken = default)
        where T : class
    {
        if (request == null || request.Data == null)
        {
        ...
        if (streamData.Length > 0)
        {
            ...
            string pathToSave = Path.Combine(root, folderName);
            Directory.CreateDirectory(pathToSave);
