...
internal class TenantService : ITenantService
{
    ...
    public async Task<bool> ExistsWithEmailAsync(string email) =>
        (await _tenantStore.GetAllAsync()).Any(t => t.AdminEmail == email);
    ...
    public async Task<bool> UpdateEmailAsync(string email)
    {
        ...
        {
            throw new InternalServerException(string.Format(_localizer["tenant.invalid"], email));
        }

        string id = _currentTenant.Id;

        if ((await _tenantStore.GetAllAsync()).Any(t => t.AdminEmail == email && t.Id != id))
        {
            throw new InternalServerException(string.Format(_localizer["tenant.alreadyexists"], email));
        }
